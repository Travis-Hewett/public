...
class User:
    db_name = "login_reg"  # Replace this with the name of your schema name
    ...
    def get_user_by_id(cls, data):
        ...
        if len(results) == 0:
            ...
            # Need [0] to grab a dictionary from the list called "results"
    ...
    def validate_registration(form_data):
        ...
        email_data = {
            # New dictionary with just the email only
    ...
    @staticmethod
    def validate_registration_update(form_data):
        print(form_data)
        is_valid = True  # Assume FOR NOW everything is good
        # Check each field individually
        if len(form_data["first_name"]) < 3:
    ...
    def validate_login(form_data):
        ...
        if found_user_or_none == None:  # No user found - that means a value of None
            # Leave intentionally vague message to not tip off hackers
